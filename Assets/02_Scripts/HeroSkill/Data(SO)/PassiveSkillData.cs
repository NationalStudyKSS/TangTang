...
public class PassiveSkillData : ScriptableObject
{
    ...
    Dictionary<PassiveSkillStatType, PassiveSkillLevelStat> _levelStatMap = new Dictionary<PassiveSkillStatType, PassiveSkillLevelStat>();
    // = new();
    int _maxLevel; // 최대 레벨
    ...
    /// </summary>
    /// <param name="statType">패시브스킬 스탯 종류</param>
    /// <param name="level">패시브스킬 레벨</param>
    /// <returns></returns>
    public float GetStat(PassiveSkillStatType statType, int level)
    {
        ...
        Debug.LogWarning($"{_passiveSkillName} 패시브스킬은 {statType} 스텟이 없습니다.");
        return 0;
    }
